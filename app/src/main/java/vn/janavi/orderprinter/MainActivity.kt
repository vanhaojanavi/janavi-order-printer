...
/**
 * App bọc ngoài (wrapper) chạy trên máy quét mã của nhân viên (Sunmi L2/T2): hiển thị trang quầy
  * (staff/index.html của order-app) hoặc trang khách tự order (order/index.html) trong WebView, và
   * cấp cho trang web đó khả năng in hóa đơn qua máy in nhiệt tích hợp sẵn của Sunmi (nếu có) hoặc
    * máy in nhiệt Bluetooth rời.
     *
      * Cách dùng:
       * - Mở app lần đầu sẽ hỏi địa chỉ server (vd http://192.168.4.1:8020/staff/index.html)
        * - Vào Cài đặt Bluetooth của Android, ghép đôi (pair) với máy in nhiệt trước (nếu dùng máy in rời)
         * - Trong app này bấm "Chọn máy in" để chọn đúng máy in đã ghép đôi ở trên
          * - Nút "Đổi server" dùng khi cần đổi địa chỉ server (đổi wifi, đổi máy chủ...)
           * - Nút "Màn nhân viên" / "Màn khách tự order" dùng để chuyển qua lại 2 màn hình
            */
                ...
                @SuppressLint("SetJavaScriptEnabled")
                    override fun onCreate(savedInstanceState: Bundle?) {
                                                ...
                                                findViewById<android.widget.Button>(R.id.btnServerSettings).setOnClickListener {
                                                        ...
                                                        findViewById<android.widget.Button>(R.id.btnPrinterSettings).setOnClickListener {
                                                                ...
                                                                findViewById<android.widget.Button>(R.id.btnLabelPrinterSettings).setOnClickListener {
                                                                        ...
                                                                        findViewById<android.widget.Button>(R.id.btnStaffScreen).setOnClickListener {
                                                                                        switchScreen("/staff/index.html")
                                                                        }
                                                                                findViewById<android.widget.Button>(R.id.btnCustomerScreen).setOnClickListener {
                                                                                                switchScreen("/order/index.html")
                                                                                }
                        ...
                        override fun onDestroy() {
                                    printerBridge.onDestroy()
                                            super.onDestroy()
                        }

                            private fun switchScreen(path: String) {
                                        val current = prefs.getString(KEY_SERVER_URL, DEFAULT_URL) ?: DEFAULT_URL
                                        val root = Regex("^(https?://[^/]+)").find(current)?.groupValues?.get(1)
                                                if (root.isNullOrBlank()) {
                                                                Toast.makeText(this, "Chưa có địa chỉ server - bấm \"Đổi server\" trước", Toast.LENGTH_LONG).show()
                                                                            return
                                                }
                                                        val url = root + path
